...
public class DataColumn {
	 ...
	 */
	private List<?> makeList(Integer type, Integer varidx, List<Integer> obindex) {
		if (type <= 32768) {
			...
			for(Integer ob : obindex) tmp.add(Data.getNum(varidx, ob));
			return tmp;
		} else if (type == 65527) {
			List<Float> tmp = new ArrayList<>();
			for(Integer ob : obindex) tmp.add((float) Data.getNum(varidx, ob));
			return tmp;
		} else if (type == 65528) {
			List<Integer> tmp = new ArrayList<>();
			for(Integer ob : obindex) tmp.add((int) Data.getNum(varidx, ob));
			return tmp;
		} else if (type == 65529) {
			List<Short> tmp = new ArrayList<>();
			for(Integer ob : obindex) tmp.add((short) Data.getNum(varidx, ob));
			return tmp;
		} else {
			List<Byte> tmp = new ArrayList<>();
			for(Integer ob : obindex) tmp.add((byte) Data.getNum(varidx, ob));
			return tmp;
		}
	}
