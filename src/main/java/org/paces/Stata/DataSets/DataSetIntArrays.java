...
public class DataSetIntArrays implements StataData {
	...
	/***
	 * Constructor method for DataSetIntArrays class
	 ...
	 */
	public DataSetIntArrays(Meta metaobject, Integer missingValue) {
		this(metaobject);
	...
	public void setData() {
		...
		Integer[][] obs = new Integer[this.metaob.getObsindex().size()][this.metaob.getVarindex().size()];

		for (Integer i = 0; i < metaob.getObsindex().size(); i++) {

			// Loop over the variable indices
			for (Integer j = 0; j < metaob.getVarindex().size(); j++) {
				...
				if (Data.isValueMissing(Data.getNum(metaob.getVarindex(j), i))) {
					...
					obs[i][j] = ((int)Math.round(Data.getNum(this.metaob.getVarindex(j), i) / 1.0D));
