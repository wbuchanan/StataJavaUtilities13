...
public class DataSetLongArrays implements StataData {
	 ...
	 */
	private Long missing = -1L;

	/***
	 * Constructor method for DataSetLongArrays class
	 ...
	 */
	public DataSetLongArrays(Meta metaobject, Long missingValue) {
		this(metaobject);
		...
		for (Integer i = 0; i < metaob.getObsindex().size(); i++) {
			...
			for (Integer j = 0; j < metaob.getVarindex().size(); j++) {
				...
				if (Data.isValueMissing(Data.getNum(metaob.getVarindex(j), i))) {
					...
					obs[i][j] = Math.round(Data.getNum(this.metaob.getVarindex(j), i) / 1.0D);
